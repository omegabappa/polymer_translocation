...
//	int frames=1;      // no of frames in a successful trajectory
	...
	file2=fopen("waiting_time_s_N2.dat","w"); // file where a single traj waiting time is stored
...
// Reading a single frame in the trajectory
		for(k=0;k<=90;k++)                        // coordinate of 117 beads in a frames
		{
		fscanf(file1,"%d %f %f %f \n", &atom_no[k], &x[k], &y[k], &z[k] ); 
//		printf( "dum_x is %f \n", dum_x);
//		x[10-k]=dum_x;
...
////////////*************************  exceptions
					...
					else if(s==91.0 && x[s-1]<0.5)            //CHANGE
					{
					s_final=91.0 - 1.0 + 0.5+x[s-1];          //CHANGE
					}
					else if(s=91.0 && x[s-1]>0.5)            //CHANGE
					{
					s_final=91.0;
		...
		for(k=91;k<=106;k++)                        // coordinate of 117 beads in a frames
		{
